...
void Credits::init(int id)
{
    for (int i = 0; i < 17; i++)
    {
        delete texts[i];
        texts[i] = TextManager::getInstance()->getText(1704 + i);
    }
    ...
    for (int i = 0; i < 6; i++)
    {
        ResourceManager::getInstance()->free(images[i]);
        os2 << (i + 1);
        string filename = "data/images/ending/ending_" + os1.str() + "/credits/credits_" + os2.str() + ".png";
...
void Credits::draw()
{
    ...
    case 8:
        ...
        if (anim >= 128)
        {
            ...
            texts[9]->display(160 - (size / 2), 20);
        }
        if (anim >= 256)
        {
            size = texts[10]->getLength() * letterSize;
            texts[10]->display(160 - (size / 2), 80);
        }
        if (anim >= 384)
        {
            size = texts[11]->getLength() * letterSize;
            texts[11]->display(160 - (size / 2), 120);
        }
        if (anim >= 512)
        {
            size = texts[12]->getLength() * letterSize;
            texts[12]->display(160 - (size / 2), 160);
        }
        break;
    case 9:
        WindowManager::getInstance()->draw(images[4], 0, 0, 320, 240, 0, 0);
    ...
    case 10:
        ...
        if (anim >= 128)
        {
            ...
            texts[13]->display(160 - (size / 2), 20);
        }
        if (anim >= 256)
        {
            size = texts[14]->getLength() * letterSize;
            texts[14]->display(160 - (size / 2), 80);
        }
        if (anim >= 384)
        {
            size = texts[15]->getLength() * letterSize;
            texts[15]->display(160 - (size / 2), 100);
        }
        if (anim >= 512)
        {
            size = texts[16]->getLength() * letterSize;
            texts[16]->display(160 - (size / 2), 120);
        }
        break;
    }
}
