...
void Save::load()
{
    ...
    for (int i = 0; i < NB_TROC_GEMME; i++)
        f.read((char *)&trocGemme[i], sizeof(int));
    for (int i = 0; i < NB_GEMME_FORCE; i++)
        f.read((char *)&gemmeForce[i], sizeof(int));
    for (int i = 0; i < NB_QUART_COEUR; i++)
    ...
    for (int i = 0; i < NB_TROC; i++)
        f.read((char *)&troc[i], sizeof(int));
    for (int i = 0; i < NB_POISSONS; i++)
        f.read((char *)&poissons[i], sizeof(int));
    for (int j = 0; j < COFFRES_MAX_SIZE; j++)
        for (int i = 0; i < NB_COFFRES; i++)
...
void Save::save(bool endGame)
{
    ...
    for (int i = 0; i < NB_TROC_GEMME; i++)
        f.write((char *)&trocGemme[i], sizeof(int));
    for (int i = 0; i < NB_GEMME_FORCE; i++)
        f.write((char *)&gemmeForce[i], sizeof(int));
    for (int i = 0; i < NB_QUART_COEUR; i++)
    ...
    for (int i = 0; i < NB_TROC; i++)
        f.write((char *)&troc[i], sizeof(int));
    for (int i = 0; i < NB_POISSONS; i++)
        f.write((char *)&poissons[i], sizeof(int));
    for (int j = 0; j < COFFRES_MAX_SIZE; j++)
        for (int i = 0; i < NB_COFFRES; i++)
...
int Save::getCompletion()
{
    ...
    for (int i = 0; i < NB_MONSTERS; i++)
        if (monsters[i] != 0)
            tmp++; // 100 -> 430
    tmp += maxRupees == 999 ? 4 : maxRupees == 500 ? 3
                              : maxRupees == 200   ? 2
                              : maxRupees == 99    ? 1
                                                   : 0;
    tmp += maxGanonOr == 999 ? 4 : maxGanonOr == 500 ? 3
                               : maxGanonOr == 200   ? 2
                               : maxGanonOr == 99    ? 1
                                                     : 0;
    tmp += maxPieceOr == 999 ? 4 : maxPieceOr == 500 ? 3
                               : maxPieceOr == 200   ? 2
                               : maxPieceOr == 99    ? 1
                                                     : 0; // 12 -> 442
    ...
    for (int i = 0; i < NB_TROC; i++)
        if (troc[i] > -1)
            tmp++; // 16 -> 476
    for (int i = 0; i < NB_TROC_GEMME; i++)
        tmp += trocGemme[i]; // 29 -> 505
    if (trocGemme[0] >= TGR_RIEN)
        tmp--;
    if (trocGemme[0] >= TGR_RIEN_2)
        tmp--;
    for (int i = 0; i < NB_POISSONS; i++)
        if (poissons[i] != 0)
            tmp++; // 30 -> 535
