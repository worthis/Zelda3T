...
void Teleport::end()
{
    ...
    if (dstMap == 57 && dstMap != currentMap && dstY == 25 * 16 + 8)
    { // generique
    ...
    else if (dstMap >= 46 && dstMap < 61 && !etage && dstMap != currentMap && (dstMap != 57 || scene->getAvancement() >= AV_TELE_ZELDA) && (dstMap != 58 || scene->getLink()->getInventory()->hasObject(TRIFORCE)))
    {
