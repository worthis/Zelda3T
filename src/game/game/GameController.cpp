...
void GameController::draw()
{
    ...
    if (transition.isRunning())
    {
        ...
        else if (transition.isAchieved())
        {
            ...
            if (step == GAME_MAIN)
            {
                if (scene.getScene()->getMap()->getId() == 57 && scene.getScene()->getAvancement() == AV_RETOUR_PRESENT)
                    displayText(1332);
            }
        }
    }
