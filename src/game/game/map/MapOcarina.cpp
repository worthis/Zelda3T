...
void MapOcarina::loop()
{
    ...
    if (teleport && !AudioManager::getInstance()->isPlaying(2))
    {
        ...
        int offsetTime = scene->getMap()->getEpoque() == EP_FUTUR ? 30 : scene->getMap()->getEpoque() == EP_PRESENT ? 15
                                                                                                                    : 0;
        switch (position)
        {
        case 0:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(8 + offsetTime, 39 * 16 + 8, 14 * 16, S, true, true);
            break;
        case 1:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(7 + offsetTime, 84 * 16, 12 * 16, S, true, true);
            break;
        case 2:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(6 + offsetTime, 62 * 16 + 8, 15 * 16, S, true, true);
            break;
        case 3:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(13 + offsetTime, 42 * 16 + 8, 38 * 16, S, true, true);
            break;
        case 4:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(1 + offsetTime, 39 * 16 + 8, 42 * 16, S, true, true);
            break;
        case 5:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(3 + offsetTime, 39 * 16 + 8, 53 * 16, S, true, true);
            break;
        case 6:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(12 + offsetTime, 45 * 16, 40 * 16, S, true, true);
            break;
        case 7:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(11 + offsetTime, 72 * 16 + 8, 60 * 16, S, true, true);
            break;
        case 8:
            MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(10 + offsetTime, 50 * 16, 31 * 16, S, true, true);
            break;
        default:
            return;
        }
