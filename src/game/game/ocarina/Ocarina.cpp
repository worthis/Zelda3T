...
void Ocarina::stop() {
    ...
    if (id == 0) AudioManager::getInstance()->replayMusic();

    Inventory* inventory = link->getInventory();
    switch (id) {
        case 0 : if (!inventory->hasObject(CHANT_1)) {link->trouveObjet(TI_CHANT_1); return;} break;
        case 1 : if (!inventory->hasObject(CHANT_2)) {link->trouveObjet(TI_CHANT_2); return;} break;
        case 2 : if (!inventory->hasObject(CHANT_3)) {link->trouveObjet(TI_CHANT_3); return;} break;
        default : break;
    }
