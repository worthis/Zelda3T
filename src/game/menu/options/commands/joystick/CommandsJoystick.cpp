...
void CommandsJoystick::init() {
    ...
    texts[15] = tm->getText(43);
    texts[16] = tm->getText(44);
    texts[17] = tm->getText(45);
    texts[18] = tm->getText(55);
    texts[19] = tm->getText(56);
...
void CommandsJoystick::reloadTextsForKeys() {
    ...
    for (int i = 20; i < 32; i++) {
        delete texts[i];
        texts[i] = jb->getTextForType(getBindableTypeForId(i - 20));
    }
}
...
BindableType CommandsJoystick::getBindableTypeForId(int i) {
    switch (i) {
        ...
        case 5 : return BT_OPTIONS;
        case 6 : return BT_CARTE;
        case 7 : return BT_MONSTERS;
        case 8 : return BT_TROC;
        case 9 : return BT_POISSONS;
        case 10 : return BT_ONILINK;
        case 11 : return BT_TELEPORT;
...
void CommandsJoystick::drawConfirm() {//104, 84
    ...
    int size = texts[17]->getLength() * letterSize;
    texts[17]->display(104 + (112 - size) / 2, 84 + 9);

    // yes
    size = texts[18]->getLength() * letterSize;
    texts[18]->display(104 + (112 - size) / 2, 84 + 29);

    // no
    size = texts[19]->getLength() * letterSize;
    texts[19]->display(104 + (112 - size) / 2, 84 + 49);
}
...
void CommandsJoystick::drawPage() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        size = texts[20 + i + page * 6]->getLength() * letterSize;
        texts[20 + i + page * 6]->display(272 - size, 72 + 16 * i);
    }
    ...
    texts[16]->display(168 + (136 - size) / 2, 200);
}
