...
void CommandsKeyboard::init() {
    ...
    texts[2] = tm->getText(61);
    texts[3] = tm->getText(62);
    texts[4] = tm->getText(63);
    ...
    texts[17] = tm->getText(40);
    texts[18] = tm->getText(41);
    texts[19] = tm->getText(42);
    texts[20] = tm->getText(43);
    texts[21] = tm->getText(60);
    texts[22] = tm->getText(44);
    texts[23] = tm->getText(45);
    texts[24] = tm->getText(55);
    texts[25] = tm->getText(56);
...
void CommandsKeyboard::reloadTextsForKeys() {
    ...
    for (int i = 26; i < 43; i++) {
        delete texts[i];
        texts[i] = kb->getTextForType(getBindableTypeForId(i - 26));
    }
}
...
BindableType CommandsKeyboard::getBindableTypeForId(int i) {
    switch (i) {
        ...
        case 9 : return BT_OPTIONS;
        case 10 : return BT_CARTE;
        case 11 : return BT_MONSTERS;
        case 12 : return BT_TROC;
        case 13 : return BT_POISSONS;
        case 14 : return BT_ONILINK;
        case 15 : return BT_TELEPORT;
        case 16 : return BT_CAMERA;
...
void CommandsKeyboard::drawConfirm() {//104, 84
    ...
    int size = texts[23]->getLength() * letterSize;
    texts[23]->display(104 + (112 - size) / 2, 84 + 9);

    // yes
    size = texts[24]->getLength() * letterSize;
    texts[24]->display(104 + (112 - size) / 2, 84 + 29);

    // no
    size = texts[25]->getLength() * letterSize;
    texts[25]->display(104 + (112 - size) / 2, 84 + 49);
}
...
void CommandsKeyboard::drawPage() {
    ...
    for (int i = 0; i < 6; i++) {
        if (page < 2 || i < 5) {
            ...
            size = texts[26 + i + page * 6]->getLength() * letterSize;
            texts[26 + i + page * 6]->display(272 - size, 72 + 16 * i);
        }
    }
    ...
    texts[22]->display(168 + (136 - size) / 2, 200);
}
