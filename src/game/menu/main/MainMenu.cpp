...
void MainMenu::drawSaves()
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (save->isLoaded())
        {
            ...
            if (save->getCristaux(0) || save->getCristaux(1) || save->getCristaux(2) || save->getCristaux(3) || save->getCristaux(4) || save->getCristaux(5) || save->getCristaux(6))
            {
                for (int j = 0; j < NB_CRISTAUX; j++)
                {
                    WindowManager::getInstance()->draw(stuff, save->getCristaux(j) ? 64 : 80, 96, 16, 16,
                                                       176 - 28 + ((16 + 2) * j),
                                                       56 + i * 48);
                }
            }
            ...
            {
                if (save->getAvancement() >= AV_STELE_LUE)
                {
                    for (int j = 0; j < NB_MEDAILLONS; j++)
                    {
                        WindowManager::getInstance()->draw(stuff, save->getMedaillons(j) ? j * 16 : 48, 64, 16, 16,
                                                           176 - 28 + ((16 + 2) * j),
                                                           56 + i * 48);
                    }
                    if (save->getEquipment(CLE_TEMPS))
                    {
                        WindowManager::getInstance()->draw(stuff, 80, 64, 16, 16, 202, 56 + i * 48);
                    }
                    if (save->getEquipment(OCARINA))
                    {
                        WindowManager::getInstance()->draw(stuff, 112, 96, 16, 16, 220, 56 + i * 48);
                    }
                }
            }
            ...
            {
                WindowManager::getInstance()->draw(stuff, 66, 48, 17, 16, 280, 56 + i * 48);
            }
        }
    }
}
