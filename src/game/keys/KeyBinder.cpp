...
Keys KeyBinder::getDefaultKey(BindableType type) {
    switch (type) {
        ...
        case BT_TROC: return kI;
        case BT_POISSONS: return kU;
...
Text* KeyBinder::getTextForKey(Keys key, int color) {
    ...
    switch (key) {
        ...
        case kEscape : tmp = TextManager::getInstance()->getText(46); break;
        case kTab : tmp = TextManager::getInstance()->getText(47); break;
        case kShift : tmp = TextManager::getInstance()->getText(48); break;
        case kCtrl : os << "CTRL"; break;
        case kAlt : os << "ALT"; break;
        case kSpace : tmp = TextManager::getInstance()->getText(49); break;
        case kReturn : tmp = TextManager::getInstance()->getText(50); break;
        case k0 : os << "0"; break;
        case k1 : os << "1"; break;
        case k2 : os << "2"; break;
        case k3 : os << "3"; break;
        ...
        case k8 : os << "8"; break;
        case k9 : os << "9"; break;
        case kUp : tmp = TextManager::getInstance()->getText(51); break;
        case kDown : tmp = TextManager::getInstance()->getText(52); break;
        case kLeft : tmp = TextManager::getInstance()->getText(53); break;
        case kRight : tmp = TextManager::getInstance()->getText(54); break;
...
Action* KeyBinder::getActionForGame(Event* event) {
    ...
    action.setAction(TROC, event->isPushed(map[BT_TROC]));
    action.setAction(POISSONS, event->isPushed(map[BT_POISSONS]));
    action.setAction(INVENTAIRE, event->isPushed(map[BT_INVENTAIRE]));
    action.setAction(QUIT_GAME, event->isPushed(map[BT_OPTIONS]));

    action.setAction(ONILINK, event->isPushed(map[BT_ONILINK]));
