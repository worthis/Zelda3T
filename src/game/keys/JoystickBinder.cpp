...
Keys JoystickBinder::getDefaultKey(BindableType type) {
    switch (type) {
        ...
        case BT_TROC: return jRT;
        case BT_POISSONS: return jLT;
        ...
        case BT_ONILINK: return jRS;
        case BT_TELEPORT: return jLS;
...
void JoystickBinder::init(string filename) {
    ...
    if(f.is_open()) {
        ...
        f.read((char *)&map[BT_TROC],sizeof(int));
        f.read((char *)&map[BT_POISSONS],sizeof(int));
        ...
        f.read((char *)&map[BT_ONILINK],sizeof(int));
...
void JoystickBinder::save() {
    ...
    f.write((char *)&map[BT_TROC],sizeof(int));
    f.write((char *)&map[BT_POISSONS],sizeof(int));
    ...
    f.write((char *)&map[BT_ONILINK],sizeof(int));
...
void JoystickBinder::addGameEventsForJoystick(Event* event) {
    ...
    if (event->isDown(map[BT_TROC])) {
        event->setEvent(kb->getKey(BT_TROC), true);
    }
    if (event->isDown(map[BT_POISSONS])) {
        event->setEvent(kb->getKey(BT_POISSONS), true);
    }
    ...
    if (event->isDown(map[BT_ONILINK])) {
        event->setEvent(kb->getKey(BT_ONILINK), true);
    }
...
bool JoystickBinder::isJoystickInUsage(Event* event) {
    return event->isPushed(map[BT_COURSE]) ||
        ...
        event->isDown(map[BT_TROC]) ||
        event->isDown(map[BT_POISSONS]) ||
        event->isDown(map[BT_ACTION]) ||
        event->isDown(map[BT_EPEE]) ||
        event->isDown(map[BT_OBJET]) ||
        event->isDown(map[BT_ONILINK]) ||
