...
ProjArrow::ProjArrow(int i, int j, Direction dir, int type) : moving(true), speed(8), dx(0), dy(0), level(0)
{
    ...
    case 3:
        effect = TE_FEU;
        force = 10;
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/arrowFire.png", true);
        break;
    case 4:
        effect = TE_GLACE;
        force = 7;
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/arrowIce.png", true);
        break;
    case 5:
        effect = TE_LUMIERE;
        force = 20;
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/arrowLight.png", true);
        break;
    case 6:
        effect = TE_MORT;
        force = 1;
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/arrowGold.png", true);
        break;
    }
}
...
void ProjArrow::projLoop()
{
    ...
    if (moving)
    {
        ...
        if (!level && scene->testDegat(&bb, direction, force, TA_ARROW, effect))
        {
            alive = false;
            return;
        }
