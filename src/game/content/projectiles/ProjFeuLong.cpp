...
ProjFeuLong::ProjFeuLong(int i, int j, Direction dir, bool bl, bool expo) : speed(4), dx(0), dy(0), level(0), force(4), byLink(bl)
{
    x = i;
    y = j;
    direction = dir;
    ...
    if (expo)
    {
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/feuExpo.png", true);
    }
    else
    {
    ...
    effect = expo ? TE_EXPONENTIEL : TE_FEU;
...
void ProjFeuLong::projLoop()
{
    ...
    if (!level && ((byLink && scene->testDegat(&bb, direction, force, TA_MAGIC, effect)) || (!byLink && scene->testDegatOnLink(&bb, direction, force, TA_MAGIC, effect))))
    {
        alive = false;
        return;
    }
