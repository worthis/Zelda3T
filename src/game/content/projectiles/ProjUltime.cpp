...
ProjUltime::ProjUltime(int i, int j, Link* lk, bool l, BoundingBox* sb) : dx(0), dy(0), parcouru(0), retour(false), link(lk), locked(l), safeBox(sb) {
    x = i - 8;
    y = j - 8;
    longX = i;
    longY = j;
...
BoundingBox* ProjUltime::getBoundingBox() {
    box.setX(longX - 8); box.setY(longY - 8);
    return &box;
}
...
void ProjUltime::computeDxDy() {
    ...
    if ((destx-x) == 0) {dx=0; dy=4;}
    else if ((desty-y) == 0) {dx=4; dy=0;}
    else {
