...
void ProjBouleExpo::projLoop() {
    ...
    if (retour) {
        ...
        if (scene->testDegatOnLink(getBoundingBox(), direction, force, TA_MAGIC, TE_EXPONENTIEL)
             || scene->testDegat(getBoundingBox(), direction, force, TA_MAGIC, TE_EXPONENTIEL, false)) {
            alive = false;
            return;
        }
    }
