...
ProjBouleFeu::ProjBouleFeu(int i, int j, double dx, double dy, BoundingBox *safeB, bool fromLk) : dx(dx), dy(dy), parcouru(0), retour(false), safeBox(safeB), fromLink(fromLk)
{
    x = i - 3;
    y = j - 3;
    longX = i;
...
void ProjBouleFeu::projLoop()
{
    ...
    {
        if (scene->testDegatOnLink(getBoundingBox(), direction, force, TA_MAGIC, TE_FEU) || scene->testDegat(getBoundingBox(), direction, force, TA_MAGIC, TE_FEU, fromLink))
        {
            alive = false;
            return;
        }
