...
ProjArcMagique::ProjArcMagique(int i, int j, Direction dir, bool expo) : speed(4), dx(0), dy(0), level(0), retour(false)
{
    x = i;
    y = j;
    direction = dir;
    ...
    if (expo)
    {
        image = ResourceManager::getInstance()->loadImage("data/images/projectiles/arcExpo.png", true);
    }
    else
    {
    ...
    effect = expo ? TE_EXPONENTIEL : TE_NORMAL;
...
void ProjArcMagique::projLoop()
{
    ...
    if (scene->checkCollisions(getBoundingBox(), (Collisionable *)this, false, false, false, level))
    {
        ...
        if (!level && ((retour && scene->testDegat(getBoundingBox(), direction, force, TA_MAGIC, effect)) || (!retour && scene->testDegatOnLink(getBoundingBox(), direction, force, TA_MAGIC, effect))))
        {
            alive = false;
            return;
        }
