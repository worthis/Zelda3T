...
Ennemi123::Ennemi123(int i, int j) : anim(0), animMax(1), vanim(180)
{
    image = ResourceManager::getInstance()->loadImage("data/images/ennemis/ennemi123.png", true);
    chrono.reset();
...
void Ennemi123::ennLoop()
{
    ...
    if (dist <= maxDist)
    {
        ...
        testDegatOnLink(&weapon, direction, forceEnn, TA_PHYSIC, TE_ARGENT);
    }
    else
    {
        idle = true;
...
void Ennemi123::draw(int offsetX, int offsetY)
{
    if (!alive)
    {
        return;
...
BoundingBox *Ennemi123::getBoundingBox()
{
    box.setX(x + 3);
    box.setY(y + 10);
    return &box;
