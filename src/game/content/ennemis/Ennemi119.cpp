...
Ennemi119::Ennemi119(int i, int j) : anim(0), animMax(1), vanim(180) {
    image = ResourceManager::getInstance()->loadImage("data/images/ennemis/ennemi119.png", true);
    chrono.reset();
...
void Ennemi119::draw(int offsetX, int offsetY) {
    if (!alive) {
        return;
    }
...
BoundingBox* Ennemi119::getBoundingBox() {
    box.setX(x + 3);
    box.setY(y + 11);
    return &box;
}

int Ennemi119::getDown() {
    return y + 27;
}
