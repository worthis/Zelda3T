...
Ennemi134::Ennemi134(int i, int j) : anim(0), animMax(1), vanim(180) {
    image = ResourceManager::getInstance()->loadImage("data/images/ennemis/ennemi134.png", true);
    chrono.reset();
...
void Ennemi134::ennLoop() {
    ...
    if (dist <= maxDist) {
        ...
        testDegatOnLink(&weapon, direction, forceEnn, TA_PHYSIC, TE_EXPONENTIEL);
...
void Ennemi134::draw(int offsetX, int offsetY) {
    if (!alive) {
        return;
    }
...
BoundingBox* Ennemi134::getBoundingBox() {
    box.setX(x + 3);
    box.setY(y + 10);
    return &box;
}
