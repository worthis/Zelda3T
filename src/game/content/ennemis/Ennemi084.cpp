...
void Ennemi084::draw(int offsetX, int offsetY) {
    ...
    if (height == 17) {
        WindowManager::getInstance()->draw(image, (direction % 2) * width + (gel ? 32 : 0), 34, width, height, dstX, dstY);
    } else {
        WindowManager::getInstance()->draw(image, 2, 51, 12, 6, dstX, dstY + height - 6); // shadow
        WindowManager::getInstance()->draw(image, (direction % 2) * width + (gel ? 32 : 0), anim * 17, 16, 16, dstX, dstY);
    }
}
