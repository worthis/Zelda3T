...
PieceOrUn::PieceOrUn(int i, int j)
{
...
void PieceOrUn::draw(int offsetX, int offsetY)
{
    if (!alive)
    {
        return;
    ...
    WindowManager::getInstance()->draw(image, 176, 64, 16, 16, dstX, dstY);
}

void PieceOrUn::action()
{
    if (!ready)
        return;
    AudioManager::getInstance()->playSound(TS_RUPEE);
    getLink()->getStatus()->updatePieceOr(1);
    alive = false;
}
