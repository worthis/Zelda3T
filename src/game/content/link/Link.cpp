...
int Link::getDefense() {
    ...
    if (tunique == 4) defense--;
    if (getBouclier() == 4) defense--;
...
void Link::handleAction(Action* action) {
    ...
    if ((animation != IDLE && animation != WALK
         && animation != FLOTTE && animation != NAGE
         && animation != BARQUE_IDLE && animation != BARQUE
         && animation != PUSH && charge == 0) || porte != 0) {
        isAble = false;
    }
    ...
    if (isAble && animation != FLOTTE && animation != NAGE && animation != BARQUE_IDLE && animation != BARQUE
        && action->isAction(HIT) && epee != 0 && !lapin) {
    ...
    if (((isAble && animation != FLOTTE && animation != NAGE && animation != BARQUE_IDLE && animation != BARQUE) || animation == PECHE)
        && action->isAction(OBJECT) && !charging && !lapin /*&& !status->isOniLink()*/) {
...
void Link::loop() {
    ...
    if (c == EAU_PROF && c2 == EAU_PROF && c3 == EAU_PROF && c4 == EAU_PROF
        && (animation == WALK || animation == IDLE || animation == PUSH || animation == TRANSPORTE || animation == PORTE)) {
        jumpInWater();
    }
        ...
        switch (animation) {
            ...
            case CHUTE :
                if (anim == 0) {
                    if (map->getId() == 21) {
                        ...
                        if (map->getId() == 48 || map->getId() == 55 || map->getId() == 68 || map->getId() == 155) { // lave
            ...
            case TIR_ARC :
                ...
                if (anim == 2 && status->getArrows()
                    && (status->getVirtualMagic() >= 8 || (equipement != FLECHE_FEU && equipement != FLECHE_GLACE))
                    && (status->getVirtualMagic() >= 16 || equipement != FLECHE_LUMIERE)) {
                    int a = 0;
                    int b = 0;
                    switch (direction) {
                    ...
                    switch (equipement) {
                        default :
                        case ARC : type = TP_ARROW; AudioManager::getInstance()->playSound(TS_SHOOT); break;
                        case ARC_FEE : type = TP_ARROW_2; AudioManager::getInstance()->playSound(TS_SHOOT); break;
                        case FLECHE_FEU : type = TP_ARROW_FIRE; AudioManager::getInstance()->playSound(TS_MAGIC); break;
                        case FLECHE_GLACE : type = TP_ARROW_ICE; AudioManager::getInstance()->playSound(TS_MAGIC); break;
                        case FLECHE_LUMIERE : type = TP_ARROW_LIGHT; AudioManager::getInstance()->playSound(TS_MAGIC); break;
                    }
            ...
            case BOIT_POTION :
                ...
                break;
            case BARQUE :
                if (anim == 0) AudioManager::getInstance()->playSound(TS_NAGE);
                break;
            case JUMP_IN_WAGON :
                if (anim == 0) {
                    setAnimation(WAGON);
...
void Link::draw(int offsetX, int offsetY) {
    ...
    switch (animation) {
        ...
        case PECHE : drawPeche(dstX2, dstY); break;
        case BARQUE_IDLE :
        case BARQUE : drawBarquee(dstX2, dstY); break;
        case JUMP_IN_WAGON : drawTouche(dstX2, dstY); break;
        case WAGON : drawWagon(dstX2, dstY); break;
        case JUMP_OUT_WAGON : drawTouche(dstX2, dstY); break;
        default : break;
    }
...
void Link::setAnimation(Animation newAnim) {
    ...
    switch (animation) { // animMax = nbAnim - 1
        ...
        case PECHE : anim = 0; animMax = 0; direction = S; vanim = 240; break;
        case BARQUE_IDLE :
            if (porte != 0) lacheObj();
            anim = 0; animMax = 1; vanim = 240;
            break;
        case BARQUE :
            if (porte != 0) lacheObj();
            anim = 0; animMax = 1; vanim = 240; AudioManager::getInstance()->playSound(TS_NAGE);
            break;
        case JUMP_IN_WAGON :
        case JUMP_OUT_WAGON :
            status->setInvincible();
...
int Link::getSoundForCollision(Collision c, int id) {
    ...
    switch (c) {
        ...
        case NEIGE : type = id == 0 ? TS_STEP_SNOW1 : TS_STEP_SNOW2; break;
        default : break;
    }
    return type;
}
...
bool Link::isAbleToChangeStuff() {
    return animation != TIR_ARC && animation != TIR_GRAPPIN && animation != TIR_BAGUETTE_FEU && animation != TIR_BAGUETTE_GLACE
        && animation != TIR_LANTERNE && animation != TIR_MARTEAU && animation != BOIT_POTION && animation != JOUE_OCARINA
        && animation != PECHE && !lapin;
}
...
void Link::trouveObjet(TypeItem type, int id) {
    ...
    switch (type) {
        ...
        case TI_TRIFORCE :
            ...
            break;
        case TI_ECREVISSE : case TI_EPERLAN : case TI_POISSON_ZEBRE : case TI_GOBIE : case TI_CRABE :
        case TI_PIBALE : case TI_VAIRON : case TI_CARASSIN_DORE : case TI_ABLETTE : case TI_GOUJON :
        case TI_LOCHE : case TI_EPINOCHE : case TI_GREMILLE : case TI_BLENNIE : case TI_MAGICARPE :
        case TI_TRUITE : case TI_AYU : case TI_PERCHE_NOIRE : case TI_ANGUILLE : case TI_POISSON_CHAT :
        case TI_BLACK_BASS : case TI_HUCHON : case TI_BARBEAU : case TI_LAMPROIE : case TI_HOTU :
        case TI_SAUMON : case TI_ESTURGEON : case TI_OMBLE_CHEVALIER : case TI_CARPE : case TI_SILURE :
    ...
    if ((animation == IDLE || animation == WALK || animation == PUSH) && charge == 0 /*&& !lapin*/) {
        ...
        if (lapin || (type == TI_QUART_COEUR || type == TI_EPEE_1 || type == TI_EPEE_2 || type == TI_EPEE_3 || type == TI_EPEE_4
...
bool Link::useStuff() {
    ...
    switch (e) {
        ...
        case CANNE_A_PECHE_3 :
            ...
            } else if (map->getSol(x, y + 23) == 67) { // ponton present
                setAnimation(PECHE);
            } else if (map->getSol(x, y + 23) == 1794) { // ponton futur
...
void Link::drawChute(int dstX, int dstY) {
    ...
    if (lapin && anim < 4) {
        switch (anim) {
            case 0 : srcX = 272; break;
            case 1 : srcX = 294; srcW = 17; dstX += 3; break;
            case 2 : srcX = 311; srcW = 13; dstX += 4; break;
            case 3 : srcX = 324; srcW = 8; dstX += 7; break;
            default : srcX = 272; break;
        }
        srcY = 173;
    } else {
        srcX = 216 + srcW * anim;
...
void Link::drawBarquee(int dstX, int dstY) {
    ...
    switch (direction) {
        case N : srcX = 204 + 34 * anim + (animation == BARQUE ? 70 : 0); srcY = 0; srcW = 32; srcH = 37; offsetX = -8; offsetY = 0; break;
        case S : srcX = 204 + 34 * anim + (animation == BARQUE ? 70 : 0); srcY = 37; srcW = 32; srcH = 41; offsetX = -8; offsetY = -9; break;
        case W : srcX = 204 + 35 * anim + (animation == BARQUE ? 70 : 0); srcY = 98; srcW = 35; srcH = 20; offsetX = -5; offsetY = 7; break;
        case E : srcX = 204 + 35 * anim + (animation == BARQUE ? 70 : 0); srcY = 78; srcW = 35; srcH = 20; offsetX = -14; offsetY = 7; break;
    }
...
void Link::drawWagon(int dstX, int dstY) {
    ...
    switch (direction) {
        case N : srcX = 272; srcY = 3; srcH = 14; offsetX = 0; offsetY = -4;
            break;
        case S : srcX = 288; srcY = 4; srcH = 20; offsetX = 0; offsetY = -8;
            break;
        case W : srcX = 304; srcY = 1; srcH = 17; offsetX = -3; offsetY = -9;
            break;
        case E : srcX = 336; srcY = 1; srcH = 17; offsetX = 3; offsetY = -9;
            break;
    }
