...
void Inventory::draw(Equipment e, int x, int y)
{
    ...
    case CANNE_A_PECHE_1:
        WindowManager::getInstance()->draw(inventaire, 32, 136, 16, 17, x, y);
        break;
    case CANNE_A_PECHE_2:
        WindowManager::getInstance()->draw(inventaire, 48, 136, 16, 17, x, y);
        break;
    case CANNE_A_PECHE_3:
        WindowManager::getInstance()->draw(inventaire, 64, 136, 16, 17, x, y);
        break;
    case FLECHE_FEU:
        WindowManager::getInstance()->draw(inventaire, 80, 136, 16, 17, x, y);
        break;
    case FLECHE_GLACE:
        WindowManager::getInstance()->draw(inventaire, 96, 136, 16, 17, x, y);
        break;
    case FLECHE_LUMIERE:
        WindowManager::getInstance()->draw(inventaire, 112, 136, 16, 17, x, y);
        break;
    case TROC_GEMME_ROUGE:
        drawTrocGemme(x, y, 0, trocGemme[0]);
        break;
    case TROC_GEMME_BLEUE:
        drawTrocGemme(x, y, 1, trocGemme[1]);
        break;
    case TROC_GEMME_JAUNE:
        drawTrocGemme(x, y, 2, trocGemme[2]);
        break;
    case TROC_GEMME_VERTE:
        drawTrocGemme(x, y, 3, trocGemme[3]);
        break;
    case CHANT_1:
        WindowManager::getInstance()->draw(inventaire, 0, 102, 16, 17, x, y);
        break;
    case CHANT_2:
        WindowManager::getInstance()->draw(inventaire, 16, 102, 16, 17, x, y);
        break;
    case CHANT_3:
        WindowManager::getInstance()->draw(inventaire, 32, 102, 16, 17, x, y);
        break;
    default:
        break;
    }
...
{
    int tmp = value;
    if (troc == 0 && (tmp == TGR_RIEN || tmp == TGR_RIEN_2))
        return;
    if (troc == 0 && tmp > TGR_RIEN_2)
        tmp--;
    if (troc == 0 && tmp > TGR_RIEN)
        tmp--;
...
bool Inventory::hasObject()
{
    for (int i = 0; i < NB_EQPMT; i++)
    {
        if (i != CARTE && i != ENCYCLOPEDIE && equipment[i] && i != SAC_TROC)
            return true;
    }
    return false;
}
