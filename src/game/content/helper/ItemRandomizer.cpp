...
TypeItem ItemRandomizer::checkWithLink(TypeItem type) {
    ...
    switch (type) {
        ...
        case TI_RUBIS_ROUGE :
            ...
            if (epoque == EP_PASSE && (!link->getStatus()->getMaxPieceOr() || link->getStatus()->getPieceOr() >= link->getStatus()->getMaxPieceOr())) {
                return TI_NO_ITEM;
            }
            if (epoque == EP_PRESENT && link->getStatus()->getRupees() >= link->getStatus()->getMaxRupees()) {
                return TI_NO_ITEM;
            }
            if (epoque == EP_FUTUR && (!link->getStatus()->getMaxGanonOr() || link->getStatus()->getGanonOr() >= link->getStatus()->getMaxGanonOr())) {
                return TI_NO_ITEM;
            }
            break;
        case TI_PETIT_COEUR :
