...
Portable::Portable() : carried(false), carriable(true), moving(false), byEnnemi(false), direction(S), dx(0), dy(0), air(0), poids(1),
...
void Portable::loop()
{
    ...
    if (air > 0 && !carried)
    {
        ...
        if (moving)
        {
            ...
            {
                if (scene->checkCollisions(b, (Collisionable *)this, false, false, false, false) && ((!byEnnemi && !scene->testDegat(b, direction, poids * (speed / 2), poids > 1 ? TA_MASSIF : TA_PHYSIC, TE_NORMAL)) || (byEnnemi && !scene->testDegatOnLink(b, direction, poids, TA_PHYSIC, TE_NORMAL))))
                {
                    x += dx;
                    y += dy;
                }
