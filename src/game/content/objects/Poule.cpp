...
#include "../ennemis/Ennemi052.h"
#include "../ennemis/Ennemi078.h"

#include "../../MainController.h"

Poule::Poule(int i, int j, int t) : type(t), anim(0), animMax(1), vanim(180), stop(0), invul(0), tolerance(3)
{
    x = i;
    y = j;
...
void Poule::draw(int offsetX, int offsetY)
{
    if (alive)
    {
        ...
        if (moving || !carried)
        {
            ...
            WindowManager::getInstance()->draw(image, 16 * (direction % 2) + 32 * type, 17 * anim, 16, 16, x - offsetX, y - offsetY - 1);
        }
        else
        {
            WindowManager::getInstance()->draw(image, 16 * (direction % 2) + 32 * type, 17 * anim, 16, 16, x - offsetX, y - offsetY);
        }
    }
}
...
void Poule::underAttack(Direction dir, int force, TypeAttack t, TypeEffect effect)
{
...
void Poule::enerve()
{
    if (alive)
    {
        ...
        {
            MainController::getInstance()->getGameController()->getSceneController()->getScene()->getMap()->addEnnemi(new Ennemi078(x, y));
        }
    }
}
