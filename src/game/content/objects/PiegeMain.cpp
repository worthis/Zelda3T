    ...
    else if (step == 1)
    {
        ...
        if (cooldown == 120)
        {
            ...
            if (b.intersect(link->getBoundingBox()) && link->getStatus()->getVirtualLife() > 0)
            {
                ...
                if (scene->getMap()->getId() == 50)
                {
                    if (scene->getMap()->getBounds()->getX() == 320 * 12)
                    {
                        MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(50, 129 * 16 + 8, 38 * 16 + 8, S, false, false, true);
                    }
                    else
                    {
                        MainController::getInstance()->getGameController()->getTeleportController()->setTeleport(50, 129 * 16 + 8, 87 * 16 + 8, N, false, false, true);
                    }
                }
